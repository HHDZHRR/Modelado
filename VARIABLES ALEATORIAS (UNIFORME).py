...
class GeneracionUniformeApp(ctk.CTk):
    def __init__(self):
        super().__init__()
    ...
    def crear_interfaz(self):
        ...
        self.ent_a = self.crear_campo(panel_entrada, "Valor mínimo (a):", "10")
        self.ent_b = self.crear_campo(panel_entrada, "Valor máximo (b):", "20")
        ...
        ctk.CTkLabel(
            panel_resultados,
            text="GENERACIÓN DE VARIABLES ALEATORIAS UNIFORMES (a, b)",
        ...
        ctk.CTkLabel(
            panel_resultados,
            text="Fórmula de transformación inversa: x = a + (b - a) * R  •  Donde R es uniforme (0,1).",
            text_color=ESTILOS.MUTED,
        ...
        self.tabla.heading("x", text="X (Variable Uniforme a,b)")
    ...
    def generar(self):
        ...
        try:
            ...
            a_val = float(self.ent_a.get())
            b_val = float(self.ent_b.get())
        ...
        if n <= 0 or col <= 0 or reng <= 0:
            messagebox.showerror("Error de entrada", "Los parámetros N, columna y renglón deben ser mayores a cero.")
            return

        if a_val >= b_val:
            messagebox.showerror("Error de entrada", "El valor mínimo 'a' debe ser menor al valor máximo 'b'.")
            return
        ...
        # 3. Generar variables uniformes y mostrarlas en la tabla
        resultados = []
        for i, r in enumerate(nums_r):
            x = a_val + (b_val - a_val) * r
            resultados.append(x)
            self.tabla.insert("", "end", values=(i + 1, f"{r:.5f}", f"{x:.5f}"))
        ...
        self.lbl_resumen.configure(
            text=f"Variables uniformes generadas con éxito en el rango [{a_val}, {b_val}] | Suma total = {total_suma:.5f}",
            text_color=ESTILOS.GREEN
        )
